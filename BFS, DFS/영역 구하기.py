...
'''
import sys
M, N, K = map(int, sys.stdin.readline().split()) # N:가로, M:세로

region = [[1 for i in range(N)] for j in range(M)]
visited = [[0 for i in range(N)] for j in range(M)]

for k in range(K):
    left_x, left_y, right_x, right_y = map(int, sys.stdin.readline().split())
    # (1,1) ~ (2,5) -> (1,1) (2,1) (3,1) (4,1) # (1~5,1~2)
    # (0,2) ~ (4,4) -> (2,0) (2,1) (2,2) (2,3) (3,0) (3,1) (3,2) (3,3) # (0)
    ...
    while q:
        ...
        for dir in range(4):
            ...
            if nx<0 or nx>=M or ny<0 or ny>=N:
                continue
            if region[nx][ny] != 1 or visited[nx][ny] != 0:
                continue
            visited[nx][ny] = 1
            q.append([nx,ny])
            tmp += 1
...
for i in range(M):
    for j in range(N):
        if region[i][j] == 1 and visited[i][j] == 0:
            BFS(i,j)
            count += 1
width.sort()
print(count)
